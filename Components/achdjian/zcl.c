...
//  }
  ...
  //return ( deviceEnabled == DEVICE_ENABLED ? TRUE : FALSE );
	...
	attrRec.attr.attrId=ATTRID_BASIC_DEVICE_ENABLED;
	callback(&attrRec);
	return ( *(uint8 *)attrRec.attr.dataPtr == DEVICE_ENABLED ? TRUE : FALSE );	
}
...
static uint8 zclProcessInReadCmd( zclIncoming_t *pInMsg ){
    ...
    for ( i = 0; i < readCmd->numAttr; i++ ){
		...
		attrRec.attr.attrId = readCmd->attrID[i];
		callback(&attrRec);
		statusRec->attrID = attrRec.attr.attrId;
		statusRec->data = attrRec.attr.dataPtr;
		statusRec->dataType = attrRec.attr.dataType;
		statusRec->status = attrRec.status;
		statusRec++;
	}
...
static uint8 zclProcessInWriteUndividedCmd( zclIncoming_t *pInMsg ){
    ...
    for ( i = 0; i < writeCmd->numAttr; i++ ){
		...
		if (attrRec.status == ZCL_STATUS_UNSUPPORTED_ATTRIBUTE){
			...
			writeRspCmd->attrList[writtingAttrIndex].status = ZCL_STATUS_UNSUPPORTED_ATTRIBUTE;
			writeRspCmd->attrList[writtingAttrIndex++].attrID = statusRec->attrID;
			break;
		}
		
		if ( statusRec->dataType != attrRec.attr.dataType ) {
			// Attribute data type is incorrect - stope here
			writeRspCmd->attrList[writtingAttrIndex].status = ZCL_STATUS_INVALID_DATA_TYPE;
			writeRspCmd->attrList[writtingAttrIndex++].attrID = statusRec->attrID;
			break;
		}

		if ( !zcl_AccessCtrlWrite( attrRec.accessControl ) ) {
			writeRspCmd->attrList[writtingAttrIndex].status = ZCL_STATUS_READ_ONLY;
			writeRspCmd->attrList[writtingAttrIndex++].attrID = statusRec->attrID;
			break;
		}

		if ( zcl_AccessCtrlAuthWrite( attrRec.accessControl ) ) {
			writeRspCmd->attrList[writtingAttrIndex].status = ZCL_STATUS_NOT_AUTHORIZED;
			writeRspCmd->attrList[writtingAttrIndex++].attrID = statusRec->attrID;
			break;
		}
	...
	if ( writeRspCmd->numAttr == writeCmd->numAttr ) {// All attributes can be written
		...
		for ( i = 0; i < writeCmd->numAttr; i++ ) {
			...
			if ( writeAttribute.status != ZCL_STATUS_SUCCESS ) {
				writeRspCmd->attrList[writtingAttrIndex].status = writeAttribute.status;
				writeRspCmd->attrList[writtingAttrIndex++].attrID = statusRec->attrID;
    	...
    	writeRspCmd->numAttr = writtingAttrIndex;
    	if ( writeRspCmd->numAttr  == 0 )  {
